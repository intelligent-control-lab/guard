...
def build_dynamics(dataset):
    ...
    gts = torch.cat((d, ddot), axis=1)
    train_dataset = TensorDataset(inputs[0:inputs.shape[0]*0.7], gts[0:inputs.shape[0]*0.7])
    test_dataset = TensorDataset(inputs[inputs.shape[0]*0.7:], gts[inputs.shape[0]*0.7:])
    ...
    for epoch in range(epochs):
        ...
        for batch in train_dataloader:
            ...
            loss = (model_sse + model.regurization) / batch_size
            total_loss += (model_sse.item() / batch_size)
            loss.backward()
            optimizer.step()
        print('Epoch: {}/{}, Loss: {}'.format(epoch, epochs, total_loss/len(train_dataloader)))
...
def uaissa(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0, 
    ...
    def chk_unsafe(s, point, dt_ratio, dt_adamba, env, threshold, margin, adaptive_k, adaptive_n, adaptive_sigma, trigger_by_pre_execute, pre_execute_coef, dynamics):
        ...
        safe_indexes = []
        for _ in range(5):
            output = dynamics(torch.cat((s, point)))
            ...
            if torch.isnan(d).any() or torch.isnan(ddot).any() or torch.isinf(d).any() or torch.isinf(ddot).any():
                ...
                break
            safe_indexes.append(get_safety_index(d, ddot))
    ...
    for epoch in range(epochs):
        if epoch < epochs * warmup_ratio:
            print(f"Collecting data for dynamics model...({epoch}/{epochs})")
            for t in range(local_steps_per_epoch):
                ...
                dynamics_dataset.store(o, a, robot_d, robot_ddot)
