...
class PAPOBuffer:
    """
    A buffer for storing trajectories experienced by a PAPO agent interacting
    ...
    def __init__(self, obs_dim, act_dim, size, gamma=0.99, lam=0.95):
        ...
        self.disc_adv_buf = np.zeros(size, dtype=np.float32)
    ...
    def finish_path(self, last_val=0):
        ...
        self.disc_adv_buf[path_slice] = core.discount_cumsum(deltas, self.gamma * self.lam)
    ...
    def get(self):
        ...
        disc_adv_mean, disc_adv_std = mpi_statistics_scalar(self.disc_adv_buf)
        self.disc_adv_buf = (self.disc_adv_buf - disc_adv_mean) / disc_adv_std
        data = dict(obs=torch.FloatTensor(self.obs_buf).to(device), 
                    ...
                    disc_adv=torch.FloatTensor(self.disc_adv_buf).to(device), 
...
def papo(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0, 
        ...
        k=10., omega_1=0.01, omega_2=0.01, mujoco=None, detailed=False):
    """
    Absoulute Proximal Policy Optimization (by clipping), 
    ...
    Args:
        ...
        mujoco (str): name of mujoco game (None if running continuous game).
    ...
    buf = PAPOBuffer(obs_dim, act_dim, local_steps_per_epoch, gamma, lam)

    # Set up function for computing PAPO policy loss
    def compute_loss_pi(data):
        ...
        mean_surr = torch.min(ratio*disc_adv, clipped_ratio*disc_adv).mean()

        tmp_1 = (ratio-1)*adv**2
        tmp_2 = 2*ratio*adv
        clip_tmp_1 = (clipped_ratio-1)*adv**2
        clip_tmp_2 = 2*clipped_ratio*adv
...
if __name__ == '__main__':
    ...
    if args.mujoco_name == None:
        ...
        exp_name = args.task + '_' + args.exp_name + '_' + detail + '_' + 'kl' + str(args.target_kl) + '_' + 'epochs' + str(args.epochs)
    else:
        import gymnasium
        detail = 'Detailed' if args.detailed else 'notDetailed'
        exp_name = args.mujoco_name + '_' + args.exp_name + '_' + detail + '_' + 'kl' + str(args.target_kl) + '_' + 'epochs' + str(args.epochs)
    ...
    papo(lambda : create_env(args), actor_critic=core.MLPActorCritic,
        ...
        mujoco=args.mujoco_name, k=args.k, omega_1=args.omega1, omega_2=args.omega2, detailed=args.detailed)