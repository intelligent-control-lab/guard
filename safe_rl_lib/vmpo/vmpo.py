...
class VMPOBuffer:
    """
    A buffer for storing trajectories experienced by a VMPO agent interacting
...
def vmpo(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0, 
        steps_per_epoch=4000, epochs=50, gamma=0.99, pi_lr=3e-4,
        vf_lr=1e-3, train_pi_iters=80, train_v_iters=80, lam=0.97, max_ep_len=1000,
        target_kl=0.01, logger_kwargs=dict(), save_freq=10, model_save=False,
        atari=None, eps_eta=0.1, eps_alpha=0.008):
    """
    V-MPO 
    ...
    buf = VMPOBuffer(obs_dim, act_dim, local_steps_per_epoch, gamma, lam)
    
    eta = torch.autograd.Variable(torch.tensor(1.0), requires_grad=True)
    alpha = torch.autograd.Variable(torch.tensor(5.0), requires_grad=True)

    # Set up function for computing VMPO policy loss
    ...
    def compute_loss_pi(data):
        ...
        advprobs = torch.stack((adv, logp))
        advprobs = advprobs[:, torch.sort(advprobs[0], descending=True).indices]
        good_adv = advprobs[0, :obs.shape[0]//2]
        good_logp = advprobs[1, :obs.shape[0]//2]
        
        phis = torch.exp(good_adv / eta.detach()) / torch.sum(torch.exp(good_adv / eta.detach()))
        L_pi = -(phis*good_logp).sum()
        L_eta = eta*eps_eta + eta*torch.log(torch.mean(torch.exp(good_adv / eta)))
        kl = compute_kl_pi(data, ac.pi)
        L_alpha = torch.mean(alpha*(eps_alpha - kl.detach()) + alpha.detach()*kl) 
        
        loss_pi = L_pi + L_eta + L_alpha
    ...
    pi_optimizer = Adam(params, lr=pi_lr)
    ...
    def update():
        ...
        for i in range(train_pi_iters):
            ...
            pi_optimizer.step()
            with torch.no_grad():
                eta.copy_(torch.clamp(eta, min=1e-8))
                alpha.copy_(torch.clamp(alpha, min=1e-8))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_name', type=str, default='vmpo')
    parser.add_argument('--eps_eta', type=float, default=0.008)
    ...
    if args.atari_name == None:
        exp_name = args.task + '_' + args.exp_name + '_' + str(args.eps_eta) + '_' + str(args.eps_alpha)\
            + '_' + 'kl' + str(args.target_kl) + '_' + 'epochs' + str(args.epochs)
    else:
        import gymnasium
    ...
    vmpo(lambda : create_env(args), actor_critic=core.MLPActorCritic,
        ac_kwargs=dict(hidden_sizes=[args.hid]*args.l), gamma=args.gamma, 
        seed=args.seed, steps_per_epoch=args.steps, epochs=args.epochs,
        logger_kwargs=logger_kwargs, target_kl=args.target_kl, model_save=model_save,
        atari=args.atari_name, eps_eta=args.eps_eta, eps_alpha=args.eps_alpha)