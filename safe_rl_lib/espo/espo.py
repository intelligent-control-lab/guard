...
class ESPOBuffer:
    """
    A buffer for storing trajectories experienced by a ESPO agent interacting
...
def espo(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0, 
        steps_per_epoch=4000, epochs=50, gamma=0.99, pi_lr=3e-4,
        vf_lr=1e-3, train_pi_iters=80, train_v_iters=80, lam=0.97, max_ep_len=1000,
        delta=0.01, logger_kwargs=dict(), save_freq=10, model_save=False,
        atari=None):
    """
    Early Stop Policy Optimization
    ...
    Args:
        ...
        delta (float): Roughly what KL divergence we think is appropriate
    ...
    buf = ESPOBuffer(obs_dim, act_dim, local_steps_per_epoch, gamma, lam)

    # Set up function for computing ESPO policy loss
    def compute_loss_pi(data):
        ...
        # Useful extra info
        deviations = torch.abs(ratio - 1.0).mean().item()
        approx_kl = (logp_old - logp).mean().item()
        ent = pi.entropy().mean().item()
        pi_info = dict(kl=approx_kl, ent=ent, deviations=deviations)
    ...
    def update():
        ...
        for i in range(train_pi_iters):
            ...
            dev = mpi_avg(pi_info['deviations'])
            if dev > delta:
                logger.log('Early stopping at step %d due to reaching max delta.'%i)
                break
            loss_pi.backward()
        ...
        dev, kl, ent = pi_info['deviations'], pi_info['kl'], pi_info_old['ent']
        logger.store(LossPi=pi_l_old, LossV=v_l_old,
                     KL=kl, Entropy=ent, AbsoluteRatioDeviation=dev,
    ...
    for epoch in range(epochs):
        ...
        logger.log_tabular('AbsoluteRatioDeviation', average_only=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_name', type=str, default='espo')
    ...
    if args.atari_name == None:
        exp_name = args.task + '_' + args.exp_name + '_' + 'delta' + str(args.delta) + '_' + 'epochs' + str(args.epochs)
    else:
        import gymnasium
        exp_name = args.atari_name + '_' + args.exp_name + '_' + 'delta' + str(args.delta) + '_' + 'epochs' + str(args.epochs)
    logger_kwargs = setup_logger_kwargs(exp_name, args.seed)
    
    # whether to save model
    model_save = True if args.model_save else False

    espo(lambda : create_env(args), actor_critic=core.MLPActorCritic,
        ac_kwargs=dict(hidden_sizes=[args.hid]*args.l), gamma=args.gamma, 
        seed=args.seed, steps_per_epoch=args.steps, epochs=args.epochs,
        logger_kwargs=logger_kwargs, delta=args.delta, model_save=model_save,
